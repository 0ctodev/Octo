...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CBitcoinAddress developmentAddress;
    developmentAddress.SetString(Params().DevelopmentAddress());
    CScript developmentPayee = GetScriptForDestination(developmentAddress.Get());
    ...
    CAmount developmentShare = 0.8 * COIN;
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            txNew.vout[i].nValue = masternodePayment - developmentShare;
            txNew.vout[i+1].scriptPubKey = developmentPayee;
            txNew.vout[i+1].nValue = developmentShare;

            //subtract mn payment from the stake reward
            txNew.vout[i - 1].nValue -= masternodePayment + developmentShare;
        } else {
            txNew.vout.resize(3);
            txNew.vout[1].scriptPubKey = payee;
            txNew.vout[1].nValue = masternodePayment;
            txNew.vout[2].scriptPubKey = developmentPayee;
            txNew.vout[2].nValue = developmentShare;
            txNew.vout[0].nValue = blockValue - masternodePayment - developmentShare;
        }
