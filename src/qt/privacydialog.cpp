                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOCTO ought to be enough for anybody." - Bill Gates, 2017
    ui->zOCTOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOCTOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOCTO->setEnabled(false);
        ui->pushButtonMintzOCTO->setToolTip(tr("zOCTO is currently disabled due to maintenance."));

        ui->pushButtonSpendzOCTO->setEnabled(false);
        ui->pushButtonSpendzOCTO->setToolTip(tr("zOCTO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOCTOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOCTO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOCTO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOCTO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOCTO in ") +
...
void PrivacyDialog::on_pushButtonSpendzOCTO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOCTO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOCTO
        sendzOCTO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOCTO
    sendzOCTO();
}

void PrivacyDialog::on_pushButtonZOctoControl_clicked()
{
    ZOctoControlDialog* zOctoControl = new ZOctoControlDialog(this);
    zOctoControl->setModel(walletModel);
    zOctoControl->exec();
}

void PrivacyDialog::setZOctoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOctoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOCTO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Octo Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOCTOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOCTO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OCTO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOCTO</b>";
    ...
    // use mints from zOcto selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOctoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOctoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zocto selector in case it was used
    ZOctoControlDialog::listSelectedMints.clear();
    ui->labelzOctoSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOcto Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Octo, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOcto Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOCTO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOCTO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOCTO "));
    ui->labelzOCTOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zOCTO </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zOCTO </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzOCTOSyncStatus->setVisible(fShow);
}
