...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::OCTO);
    ...
    if (!settings.contains("nAnonymizeOctoAmount"))
        settings.setValue("nAnonymizeOctoAmount", 1000);

    nAnonymizeOctoAmount = settings.value("nAnonymizeOctoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOctoAmount"))
        SoftSetArg("-anonymizeoctoamount", settings.value("nAnonymizeOctoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in octo.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOctoAmount:
            return QVariant(nAnonymizeOctoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOctoAmount:
            nAnonymizeOctoAmount = value.toInt();
            settings.setValue("nAnonymizeOctoAmount", nAnonymizeOctoAmount);
            emit anonymizeOctoAmountChanged(nAnonymizeOctoAmount);
            break;
        case CoinControlFeatures:
