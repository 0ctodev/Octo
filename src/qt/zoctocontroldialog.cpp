// Copyright (c) 2017 The OCTO developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zoctocontroldialog.h"
#include "ui_zoctocontroldialog.h"
...
std::list<std::string> ZOctoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOctoControlDialog::listMints;

ZOctoControlDialog::ZOctoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOctoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOctoControlDialog::~ZOctoControlDialog()
{
    delete ui;
}

void ZOctoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOctoControlDialog::updateList()
{
...
void ZOctoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOctoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOcto_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOctoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOctoControlDialog::GetSelectedMints()
{
...
void ZOctoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
